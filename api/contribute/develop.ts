...
export const AllProperties = AllSchemas
    .flatMap(s => s.properties)
    .reduce((agg, p) => ({ ...agg, [p.uid]: p }), {} as Record<string, Property>)

const schemaExists = (name: string) => AllSchemas.findIndex(s => s.name === name) >= 0
...
for (const prop of Object.values(AllProperties)) {
    ...
    if (prop.kind === "entityValue" && !schemaExists(prop.linkedEntitySchema)) {
        throw Error(`Linked schema ${prop.schema} does not exists`)
    }
}
...
export const isDirectChange = (change: PropertyChange): change is DirectPropertyChange => change.kind === "direct"
...
export const isOwnedEntityChange = (change: PropertyChange): change is OwnedEntityChange =>
...
// matching type.
type MatchingPropertyChangeType<T> = T extends OwnedEntityChange | LinkedEntitySelectionChange
    ? EntityValueProperty
    : (T extends EntityListChange
        ? EntityListProperty
        : Property)

const isMatchingPropertyChangeType = <T extends PropertyChange,>(prop: Property, change: T): prop is MatchingPropertyChangeType<T> => {
    if (change.kind === 'owned' || change.kind === "linked") {
        ...
        return prop.kind === "entityValue" && (change.kind !== "linked" || !prop.oneToOneBackingField)
    }
    if (change.kind === "list") {
...
const validateAndGetProperty = <T extends PropertyChange,>(change: T): MatchingPropertyChangeType<T> => {
    ...
    if (!isMatchingPropertyChangeType(prop, change)) {
        throw new Error(`Property ${property}'s type ${prop.kind} not compatible with change ${change.kind}`)
    }
    if (isOwnedEntityChange(change)) {
        const isValid = (prop as EntityValueProperty).linkedEntitySchema === change.ownedEntityId
        if (isValid) {
            throw new Error(`Owned entity is different than its corresponding property`)
        }
    }
    return prop
}
 ...
 * transaction.
 */
export const combineChangeSets = (changeSets: ChangeSet[]): CombinedChangeSet => {
    ...
    for (const u of updatedEntries) {
        ...
        for (let i = u.changes.length; i >= 0; --i) {
            ...
            if (uc.kind === "owned") {
                ...
                throw new Error(`Unknown property change kind in: ${JSON.stringify(uc)}`)
            }
        }
    }
    ...
    for (const dc of delCancelEntries) {
        ...
        if (match < 0) {
            throw new Error(`A delete cancel operation ${JSON.stringify(dc)} does not match a previous delete operation.`)
        }
    ...
    for (const u of updatedEntries) {
        const id = `${u.schema}_${typeof u.entityId === "string" ? u.entityId : u.entityId.tempId}`
