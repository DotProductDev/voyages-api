...
// basic functionality will be placed in this file.
...
const schemaExists = (name: string) =>
  AllSchemas.findIndex((s) => s.name === name) >= 0
...
// PART III - Backend implementation (Node)

interface DataFilter {
  /**
   * The field to search for, if not set, this is the PK field of the model.
   */
  field?: string
  /**
...
type NonNullFieldValue =
  | string
  | number
  | MaterializedEntity
  | MaterializedEntity[]

type FieldValue = NonNullFieldValue | null

type EntityData = Record<string, FieldValue>

interface MaterializedEntity extends EntityData {
  id: string
  schema: string
}
...
const _extractFields = (
  ...
  for (const p of properties) {
    if (
      ...
      fieldMap[p.backingField] = p.label
...
const toMap = <T>(items: T[], key: (x: T) => string) => {
...
const fetchEntities = async (
  ...
  const fieldMap = _extractFields(s.properties, options?.additionalFields)
  // Ensure that the PK field is extracted.
  fieldMap[s.pkField] ??= "id"
  const fetched = await resolver.fetch(
    {
      model: s.backingModel,
      filter
    },
    [...fieldMap.keys()]
  )
  ...
  for (const fieldValues of fetched) {
    const id = fieldValues[s.pkField]
    if (!id) {
      throw new Error(
        `The PK field '${s.pkField}' was not fetched by the data resolver`
      )
    }
    const entity = _bindEntity(s, fieldValues, fieldMap)
      ...
      if (p.kind === "entityValue") {
        ...
        if (f) {
          entity[p.label] = singleOrNull(
            await fetchEntities(p.linkedEntitySchema, [f], resolver)
          )
        }
...
const generateChangeSetQueries = (combinedChangeSet: CombinedChangeSet) => {}
